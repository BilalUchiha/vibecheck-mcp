...
submit_for_review ──► collect real change set (git-authoritative)
   ...
   │                            (retry budget: default 3, counted server-side)
   └─ approved ──► agent may now tell the user it's done
```

When the retry budget is exhausted, the agent is told to **stop and report the outstanding issues to the user** — never to loop forever, and never to describe the task as finished.
...
| `changed_files` | | Required outside git repos; otherwise cross-checked against git (a mismatch is a hard gate). |
...
Four verdicts: `approved` · `needs_fixes` · `max_retries_exceeded` · `review_unavailable` (the judge couldn't be reached — not a verdict on the change, consumes no retry).

### `configure_project`

Writes/updates `.vibecheck.json`: presets, per-dimension thresholds and toggles, retry limit, convention sampling mode, judge settings. Supports `dry_run` and `reset_attempts`.
...
- **A file claimed as changed but git disagrees** — the review ran against reality, so the claim is wrong either way

## Where the change set comes from

The server asks git rather than trusting the agent's summary:

- on a branch: diff against the merge base with the default branch, plus untracked files
- clean tree (agent committed): falls back to the most recent commit, and says so
- outside a git repo: uses the submitted `changed_files`, marked as agent-reported
